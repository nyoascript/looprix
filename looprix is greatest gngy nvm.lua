...
getgenv().LOOPRIX_CONFIG = {
    GUI_POSITION_X = nil,
    GUI_POSITION_Y = nil,
    WALK_PANEL_X = nil,
    WALK_PANEL_Y = nil,
    LOCK_TARGET_PANEL_X = nil,
    LOCK_TARGET_PANEL_Y = nil,
    LOCK_TARGET_PANEL_VISIBLE = false,
    DROP_BRAINROT_PANEL_X = nil,
    DROP_BRAINROT_PANEL_Y = nil,
...
local DEFAULT_CONFIG = table.clone(getgenv().LOOPRIX_CONFIG)

local ConfigSystem = {
    CONFIG_FILE_NAME = "LooprixDuel.json",
    CONFIG_VERSION = 4,
    SAVE_DEBOUNCE = 0.5,
    lastSaveTime = 0
}
...
-- ======================================
-- CONFIG MIGRATION & SAVE SYSTEM
-- ======================================

local function migrateConfig(savedConfig)
    if not savedConfig._version or savedConfig._version < 3 then
        if not savedConfig.gui then savedConfig.gui = {} end
        if not savedConfig.gui.main then
            savedConfig.gui.main = {
                scaleX = 0,
                scaleY = 0.5,
                offsetX = savedConfig.GUI_POSITION_X or -225,
                offsetY = savedConfig.GUI_POSITION_Y or -250
            }
        end
        if not savedConfig.gui.lockPanel then
            savedConfig.gui.lockPanel = {
                scaleX = 0,
                scaleY = 0,
                offsetX = savedConfig.LOCK_TARGET_PANEL_X or 0,
                offsetY = savedConfig.LOCK_TARGET_PANEL_Y or 0,
                visible = savedConfig.LOCK_TARGET_PANEL_VISIBLE or false
            }
        end
        if not savedConfig.gui.dropBrainrotPanel then
            savedConfig.gui.dropBrainrotPanel = {
                scaleX = 0,
                scaleY = 0,
                offsetX = savedConfig.DROP_BRAINROT_PANEL_X or 0,
                offsetY = savedConfig.DROP_BRAINROT_PANEL_Y or 0,
                visible = savedConfig.DROP_BRAINROT_PANEL_VISIBLE or false
            }
        end
        if not savedConfig.gui.speedGui then
            savedConfig.gui.speedGui = {
                scaleX = 0,
                scaleY = 0,
                offsetX = savedConfig.SPEED_GUI_X or 10,
                offsetY = savedConfig.SPEED_GUI_Y or 55,
                visible = savedConfig.SPEED_GUI_VISIBLE or false
            }
        end
        if not savedConfig.gui.floatPanel then
            savedConfig.gui.floatPanel = {
                scaleX = 0,
                scaleY = 0,
                offsetX = 0,
                offsetY = 0,
                visible = savedConfig.FLOAT_PANEL_VISIBLE or false
            }
        end
        if not savedConfig.gui.flingPanel then
            savedConfig.gui.flingPanel = {
                scaleX = 0, scaleY = 0, offsetX = 0, offsetY = 0,
                visible = savedConfig.FLING_PANEL_VISIBLE or false
            }
        end        if not savedConfig.gui.spinbotPanel then
            savedConfig.gui.spinbotPanel = {
                scaleX = 0,
                scaleY = 0,
                offsetX = savedConfig.SPINBOT_PANEL_X or 0,
                offsetY = savedConfig.SPINBOT_PANEL_Y or 0,
                visible = savedConfig.SPINBOT_PANEL_VISIBLE or false
            }
        end
        if not savedConfig.gui.tauntPanel then
            savedConfig.gui.tauntPanel = {
                scaleX = 0,
                scaleY = 0,
                offsetX = savedConfig.TAUNT_PANEL_X or 0,
                offsetY = savedConfig.TAUNT_PANEL_Y or 0,
                visible = savedConfig.TAUNT_PANEL_VISIBLE or false
            }
        end
    end
    -- v4 additions
    if not savedConfig._version or savedConfig._version < 4 then
        if not savedConfig.gui then savedConfig.gui = {} end
        if not savedConfig.gui.autoWalkPanel then
            savedConfig.gui.autoWalkPanel = {
                scaleX = 0, scaleY = 0, offsetX = 10, offsetY = 220,
                visible = false
            }
        end
        if savedConfig.toggles == nil then savedConfig.toggles = {} end
        if savedConfig.toggles.AUTO_WALK_PANEL_VISIBLE == nil then
            savedConfig.toggles.AUTO_WALK_PANEL_VISIBLE = false
        end
        if savedConfig.ui == nil then
            savedConfig.ui = { mainVisible = true }
        end
    end
    savedConfig._version = ConfigSystem.CONFIG_VERSION
    return savedConfig
end

local function buildSerializableConfig()
    local ser = {
        _version = ConfigSystem.CONFIG_VERSION,
        gui = {},
        toggles = {},
        values = {},
        keybinds = {}
    ...
    ser.gui.main = CONFIG._guiPositions and CONFIG._guiPositions.main or { scaleX = 0, scaleY = 0.5, offsetX = -225, offsetY = -250 }
    ser.gui.lockPanel = CONFIG._guiPositions and CONFIG._guiPositions.lockPanel or { scaleX = 0, scaleY = 0, offsetX = 0, offsetY = 0, visible = false }
    ser.gui.dropBrainrotPanel = CONFIG._guiPositions and CONFIG._guiPositions.dropBrainrotPanel or { scaleX = 0, scaleY = 0, offsetX = 0, offsetY = 0, visible = false }
    ser.gui.speedGui = CONFIG._guiPositions and CONFIG._guiPositions.speedGui or { scaleX = 0, scaleY = 0, offsetX = 10, offsetY = 55, visible = false }
    ser.gui.floatPanel = CONFIG._guiPositions and CONFIG._guiPositions.floatPanel or { scaleX = 0, scaleY = 0, offsetX = 0, offsetY = 0, visible = false }
    ser.gui.flingPanel = CONFIG._guiPositions and CONFIG._guiPositions.flingPanel or { scaleX = 0, scaleY = 0, offsetX = 0, offsetY = 0, visible = false }
    ser.gui.spinbotPanel = CONFIG._guiPositions and CONFIG._guiPositions.spinbotPanel or { scaleX = 0, scaleY = 0, offsetX = 0, offsetY = 0, visible = false }
    ser.gui.tauntPanel = CONFIG._guiPositions and CONFIG._guiPositions.tauntPanel or { scaleX = 0, scaleY = 0, offsetX = 0, offsetY = 0, visible = false }
    ser.gui.antiStealPanel = CONFIG._guiPositions and CONFIG._guiPositions.antiStealPanel or { scaleX = 0, scaleY = 0, offsetX = 0, offsetY = 0, visible = false }
    ser.gui.grabBar = CONFIG._guiPositions and CONFIG._guiPositions.grabBar or { scaleX = 0.5, scaleY = 0, offsetX = -90, offsetY = 140 }
    ser.gui.toggleBtn = CONFIG._guiPositions and CONFIG._guiPositions.toggleBtn or { scaleX = 0, scaleY = 1, offsetX = 10, offsetY = -50 }
    ser.gui.autoPlayMain = CONFIG._guiPositions and CONFIG._guiPositions.autoPlayMain or { scaleX = 0, scaleY = 0, offsetX = 10, offsetY = 160, visible = false }
    ser.gui.autoPlaySett = CONFIG._guiPositions and CONFIG._guiPositions.autoPlaySett or { scaleX = 0, scaleY = 0, offsetX = 220, offsetY = 160 }
    ser.gui.autoWalkPanel = CONFIG._guiPositions and CONFIG._guiPositions.autoWalkPanel or { scaleX = 0, scaleY = 0, offsetX = 10, offsetY = 220, visible = false }
    ser.ui = { mainVisible = guiVisible }
    ...
    ser.autoPlay = {
        offsets = CONFIG._autoPlayOffsets or {
    ...
    ser.keybinds = {}
    local keybindFields = {
        "TOGGLE_GUI_KEYBIND", "AUTO_BAT_KEYBIND", "AIMBOT_KEYBIND", "SPINBOT_KEYBIND",
        "LOCK_TARGET_KEYBIND", "AUTO_MEDUSA_KEYBIND", "INSTANT_GRAB_KEYBIND", 
        "DROP_BRAINROT_KEYBIND", "FLOAT_KEYBIND", "AUTO_PLAY_KEYBIND",
        "FLING_KEYBIND", "ANTI_STEAL_KEYBIND", "AUTO_WALK_KEYBIND"
    }
    for _, field in ipairs(keybindFields) do
        if CONFIG[field] then
            ser.keybinds[field] = { _type = "KeyCode", _name = CONFIG[field].Name }
        else
            ser.keybinds[field] = nil
        end
    end
...
local function loadSerializedConfig(saved)
    if saved.gui then
        if saved.gui.main then
            CONFIG._guiPositions = CONFIG._guiPositions or {}
            CONFIG._guiPositions.main = saved.gui.main
        end
        if saved.gui.lockPanel then
            CONFIG._guiPositions = CONFIG._guiPositions or {}
            CONFIG._guiPositions.lockPanel = saved.gui.lockPanel
            CONFIG.LOCK_TARGET_PANEL_VISIBLE = saved.gui.lockPanel.visible or false
        end
        if saved.gui.dropBrainrotPanel then
            CONFIG._guiPositions = CONFIG._guiPositions or {}
            CONFIG._guiPositions.dropBrainrotPanel = saved.gui.dropBrainrotPanel
            CONFIG.DROP_BRAINROT_PANEL_VISIBLE = saved.gui.dropBrainrotPanel.visible or false
        end
        if saved.gui.speedGui then
            CONFIG._guiPositions = CONFIG._guiPositions or {}
            CONFIG._guiPositions.speedGui = saved.gui.speedGui
            CONFIG.SPEED_GUI_VISIBLE = saved.gui.speedGui.visible or false
        end
        if saved.gui.floatPanel then
            CONFIG._guiPositions = CONFIG._guiPositions or {}
            CONFIG._guiPositions.floatPanel = saved.gui.floatPanel
            CONFIG.FLOAT_PANEL_VISIBLE = saved.gui.floatPanel.visible or false
        end
        if saved.gui.flingPanel then
            CONFIG._guiPositions = CONFIG._guiPositions or {}
            CONFIG._guiPositions.flingPanel = saved.gui.flingPanel
            CONFIG.FLING_PANEL_VISIBLE = saved.gui.flingPanel.visible or false
        end
        if saved.gui.spinbotPanel then
            CONFIG._guiPositions = CONFIG._guiPositions or {}
            CONFIG._guiPositions.spinbotPanel = saved.gui.spinbotPanel
            CONFIG.SPINBOT_PANEL_VISIBLE = saved.gui.spinbotPanel.visible or false
        end
        if saved.gui.tauntPanel then
            CONFIG._guiPositions = CONFIG._guiPositions or {}
            CONFIG._guiPositions.tauntPanel = saved.gui.tauntPanel
            CONFIG.TAUNT_PANEL_VISIBLE = saved.gui.tauntPanel.visible or false
        end
        if saved.gui.antiStealPanel then
            CONFIG._guiPositions = CONFIG._guiPositions or {}
            CONFIG._guiPositions.antiStealPanel = saved.gui.antiStealPanel
            CONFIG.ANTI_STEAL_PANEL_VISIBLE = saved.gui.antiStealPanel.visible or false
        end
        if saved.gui.grabBar then
            CONFIG._guiPositions = CONFIG._guiPositions or {}
            CONFIG._guiPositions.grabBar = saved.gui.grabBar
        end
        if saved.gui.toggleBtn then
            CONFIG._guiPositions = CONFIG._guiPositions or {}
            CONFIG._guiPositions.toggleBtn = saved.gui.toggleBtn
        end
        if saved.gui.autoPlayMain then
            CONFIG._guiPositions = CONFIG._guiPositions or {}
            CONFIG._guiPositions.autoPlayMain = saved.gui.autoPlayMain
            CONFIG.AUTO_PLAY_GUI_VISIBLE = saved.gui.autoPlayMain.visible or false
        end
        if saved.gui.autoPlaySett then
            CONFIG._guiPositions = CONFIG._guiPositions or {}
            CONFIG._guiPositions.autoPlaySett = saved.gui.autoPlaySett
        end
        if saved.gui.autoWalkPanel then
            CONFIG._guiPositions = CONFIG._guiPositions or {}
            CONFIG._guiPositions.autoWalkPanel = saved.gui.autoWalkPanel
            CONFIG.AUTO_WALK_PANEL_VISIBLE = saved.gui.autoWalkPanel.visible or false
        end
    end
    if saved.ui then
        if saved.ui.mainVisible ~= nil then
            guiVisible = saved.ui.mainVisible
        end
    end
    
    if saved.autoPlay then
        if type(saved.autoPlay.offsets) == "table" then
            CONFIG._autoPlayOffsets = saved.autoPlay.offsets
        end
    end
    
    if saved.toggles then
        for k, v in pairs(saved.toggles) do
            if CONFIG[k] ~= nil then CONFIG[k] = v end
        end
    end
    
    if saved.values then
        for k, v in pairs(saved.values) do
            if CONFIG[k] ~= nil then CONFIG[k] = v end
        end
    end
    
    if saved.keybinds then
        for k, v in pairs(saved.keybinds) do
            if v and v._type == "KeyCode" then
                if Enum.KeyCode[v._name] then
...
local function saveConfig()
    local now = tick()
    if now - ConfigSystem.lastSaveTime < ConfigSystem.SAVE_DEBOUNCE then
        return
    end
    ConfigSystem.lastSaveTime = now
    
    local serConfig = buildSerializableConfig()
    local success, jsonData = pcall(function()
        return game:GetService("HttpService"):JSONEncode(serConfig)
    end)
    
    if success then
        pcall(function()
            writefile(ConfigSystem.CONFIG_FILE_NAME, jsonData)
        end)
    end
end

local function loadConfig()
    if not isfile(ConfigSystem.CONFIG_FILE_NAME) then return end
    
    local readSuccess, jsonData = pcall(function()
        return readfile(ConfigSystem.CONFIG_FILE_NAME)
    end)
    
    if not readSuccess or not jsonData then return end
    
    local decodeSuccess, savedConfig = pcall(function()
        return game:GetService("HttpService"):JSONDecode(jsonData)
    end)
    
    if not decodeSuccess or not savedConfig then return end
    
    savedConfig = migrateConfig(savedConfig)
    loadSerializedConfig(savedConfig)
    ...
    -- Restore scaleMultiplier so registerScaleTarget calls during createGui() use saved value
...
local function resetConfig()
    pcall(function()
        if isfile(ConfigSystem.CONFIG_FILE_NAME) then
            delfile(ConfigSystem.CONFIG_FILE_NAME)
        end
    end)
...
local _aimbotBtn  = nil   -- mutual-exclusion toggle button refs (set in createDuelTab)
...
local function createESPForPlayer(player)
    ...
    local function setupESP(character)
        ...
        text.TextColor3 = Color3.fromRGB(220, 255, 220)
        text.TextStrokeTransparency = 0
        text.TextStrokeColor3 = Color3.fromRGB(0, 80, 0)
...
local function startESP()
    ...
    espConnection = S.RunService.RenderStepped:Connect(function()
        ...
        for player, data in pairs(espCache) do
            if player.Character and data.hrp and data.label then
                local dist = (myHRP.Position - data.hrp.Position).Magnitude
                data.label.Text = player.Name .. "  " .. math.floor(dist) .. "m"
                data.gradient.Rotation = (data.gradient.Rotation + 1) % 360
            else
                espCache[player] = nil
            end
        end
...
local function stopESP()
    ...
    for player, data in pairs(espCache) do
        if player.Character then
            local espMarker = player.Character:FindFirstChild(ESP_NAME)
            if espMarker then
                espMarker:Destroy()
            end
            local billboard = player.Character:FindFirstChild(ESP_NAME .. " Name")
            if billboard then
                billboard:Destroy()
            end
        end
...
local AW = {
    ...
    currentStep = 1,
    activeSide  = nil,
    WALK_SEQ    = {"P1","P2","P3","P4","P5"},
}
local awGuiInstance    = nil
local awWalkBtn        = nil  -- the WALK button inside the panel

local function awGetTarget(key)
    local b   = AP.BASE[AW.activeSide][key]
    local off = AP.offsets[AW.activeSide][key]
    return b + Vector3.new(off.x, 0, off.z)
end

local function awSetBtnActive(active)
    if not awWalkBtn then return end
    S.TweenService:Create(awWalkBtn, tweenInfoFast, {
    ...
    awWalkBtn.TextColor3 = active and Color3.fromRGB(10,10,10) or COLORS.TextDim
end
...
local function awLaunch()
    if AW.enabled then
        awStopLoop()
        return
    end
    local side = GetSide()
    if not side then showNotification("Auto Walk", false); return end
    AW.activeSide = side
    AW.enabled    = true
    awSetBtnActive(true)
    awStartLoop()
...
local function createAutoWalkGui()
    ...
    awWalkBtn = Instance.new("TextButton", content)
    awWalkBtn.Size = UDim2.new(1,0,0,BTN_H)
    awWalkBtn.BackgroundColor3 = COLORS.Surface
    awWalkBtn.BackgroundTransparency = COLORS.SurfaceTransparency
    awWalkBtn.Text = "▶  Walk"
    awWalkBtn.TextColor3 = COLORS.TextDim
    awWalkBtn.Font = Enum.Font.GothamBold; awWalkBtn.TextSize = 11; awWalkBtn.BorderSizePixel = 0
    Instance.new("UICorner", awWalkBtn).CornerRadius = UDim.new(0,7)
    makeAS(awWalkBtn, 1, 0.45)

    local awBadge = Instance.new("TextLabel", awWalkBtn)
    awBadge.Size = UDim2.new(0,44,0,13); awBadge.AnchorPoint = Vector2.new(1,0)
    awBadge.Position = UDim2.new(1,-3,1,-6)
    awBadge.BackgroundColor3 = COLORS.Background; awBadge.BackgroundTransparency = 0.15
    awBadge.BorderSizePixel = 0; awBadge.Font = Enum.Font.GothamBold; awBadge.TextSize = 8
    ...
    awWalkBtn.MouseButton1Click:Connect(function() awLaunch() end)
        ...
        if inp.UserInputType==Enum.UserInputType.MouseButton1 or inp.UserInputType==Enum.UserInputType.Touch then
            if awDrag then
                ...
                CONFIG._guiPositions.autoWalkPanel = {
                    ...
                    visible=CONFIG.AUTO_WALK_PANEL_VISIBLE
                }
                saveConfig()
            end
...
local function getMyPlot()
    ...
    for _, plot in ipairs(plots) do
        -- METHOD 1: Synchronizer
        local ok, channel = pcall(function()
            return S and S.Synchronizer and S.Synchronizer:Get(plot.Name)
        end)
        if ok and channel then
            local owner = pcall(function() return channel:Get("Owner") end) and channel:Get("Owner")
            if owner then
                if typeof(owner) == "Instance" and owner:IsA("Player") and owner.UserId == S.LocalPlayer.UserId then
                    return plot
                elseif typeof(owner) == "table" and owner.UserId == S.LocalPlayer.UserId then
                    return plot
                elseif typeof(owner) == "Instance" and owner == S.LocalPlayer then
                    return plot
                end
            end
        end

        -- METHOD 2: YourBase BillboardGui
...
local function getEnemyPlot(myPlot)
    if not myPlot then return nil end
    local plots = getAllPlots()
    local myPart = myPlot:FindFirstChildWhichIsA("BasePart")
    ...
    for _, plot in ipairs(plots) do
        if plot ~= myPlot then
            local part = plot:FindFirstChildWhichIsA("BasePart")
            if part then
                local d = (part.Position - myPart.Position).Magnitude
...
local function detectSide()
    ...
    if not myPlot then return nil end
    local enemyPlot = getEnemyPlot(myPlot)
    if not enemyPlot then return nil end
    local myPart    = myPlot:FindFirstChildWhichIsA("BasePart")
    local enemyPart = enemyPlot:FindFirstChildWhichIsA("BasePart")
    if not myPart or not enemyPart then return nil end
    if myPart.Position.X > enemyPart.Position.X then
        return "L"
    else
        return "R"
    end
...
-- currentSide обновляется автоматически: при старте, при спауне, каждые 5 сек
...
-- Переопределение при спауне (CharacterAdded основного модуля тоже вызывает updateSide через onCharacterAdded)
S.LocalPlayer.CharacterAdded:Connect(function()
    task.wait(1)
    updateSide()
end)

-- Фоновый опрос каждые 5 сек на случай смены команд
task.spawn(function()
    while true do
        task.wait(5)
        updateSide()
...
local function canRunAutoPlay()
    ...
    if not GetSide() then return false end
    return true
end
...
-- ======================================
-- FLOAT SYSTEM  (HipHeight lock)
-- ======================================

local floatModeConn  = nil
local floatEnabled   = false
local _floatOrigHip  = 2   -- restored on stopFloat

local function startFloat()
    if floatModeConn then return end
    floatEnabled = true

    -- Save original HipHeight before we change it
    pcall(function()
        local hum = character and character:FindFirstChildOfClass("Humanoid")
        if hum then _floatOrigHip = hum.HipHeight end
    end)
    ...
    local function applyHipHeight()
        ...
        local hum = char:FindFirstChildOfClass("Humanoid")
        if hum then
            hum.HipHeight = CONFIG.FLOAT_HEIGHT or 0
        end
    end
    applyHipHeight()

    -- Heartbeat lock: re-apply if game resets it
    floatModeConn = S.RunService.Heartbeat:Connect(function()
        if not floatEnabled then return end
        pcall(function()
            local char = character
            local hum  = char and char:FindFirstChildOfClass("Humanoid")
            if hum and math.abs(hum.HipHeight - (CONFIG.FLOAT_HEIGHT or 0)) > 0.01 then
                hum.HipHeight = CONFIG.FLOAT_HEIGHT or 0
            end
        end)
    end)
...
-- apLaunchSide: side locked once, single button toggle
local function apLaunchSide(side)
    ...
    if AP.enabled then
        ...
        AP.enabled = true
        apStartLoop()
        apSetBtnActive(apBtnL, true)
        showNotification("Auto Play", true)
    end
end
...
local function createAutoPlayGui()
    ...
    -- Single "Auto Play" button – auto-detects side via detectSide()
    local apBtn = Instance.new("TextButton", content)
    apBtn.LayoutOrder = 1
    apBtn.Size = UDim2.new(1, -PAD*2, 0, BTN_H)
    apBtn.BackgroundColor3 = COLORS.Surface
    apBtn.BackgroundTransparency = COLORS.SurfaceTransparency
    apBtn.Text = "▶  Auto Play"
    apBtn.TextColor3 = COLORS.TextDim
    apBtn.Font = Enum.Font.GothamBold; apBtn.TextSize = 11; apBtn.BorderSizePixel = 0
    Instance.new("UICorner", apBtn).CornerRadius = UDim.new(0,7)
    makeAccentStroke(apBtn, 1, 0.45)
    -- keep legacy refs so apSetBtnActive / apResetBtns still work
    apBtnL = apBtn; apBtnR = apBtn

    -- Keybind badge (bottom-right)
    local apBadge = Instance.new("TextLabel", apBtn)
    apBadge.Size = UDim2.new(0,44,0,13); apBadge.AnchorPoint = Vector2.new(1,0)
    apBadge.Position = UDim2.new(1,-3,1,-6)
    apBadge.BackgroundColor3 = COLORS.Background; apBadge.BackgroundTransparency = 0.15
    apBadge.BorderSizePixel = 0; apBadge.Font = Enum.Font.GothamBold; apBadge.TextSize = 8
    ...
    apBtn.MouseButton1Click:Connect(function() apLaunchSide(nil) end)
...
local function setupFloatingPanels()
    ...
    antiStealPanelBtn.Activated:Connect(function()
        antiStealEnabled = not antiStealEnabled
        CONFIG.ANTI_STEAL_PANEL_VISIBLE = antiStealEnabled
        antiStealPanelBtn.Text = "ANTI STEAL: " .. (antiStealEnabled and "ON" or "OFF")
        antiStealPanelBtn.TextColor3 = COLORS.Accent
        saveConfig()
        if antiStealEnabled then
            startAntiStealWatcher()
        else
            stopAntiStealWatcher()
        end
    end)
end
...
local function createBindsTab(parent)
    ...
    local autoBatKeybind = createKeybindButton(scroll, "Auto Bat Keybind", CONFIG.AUTO_BAT_KEYBIND, function(key)
        CONFIG.AUTO_BAT_KEYBIND = key
        saveConfig()
    end)
    autoBatKeybind.LayoutOrder = 1

    local aimbotKeybind = createKeybindButton(scroll, "Aimbot Keybind", CONFIG.AIMBOT_KEYBIND, function(key)  
        CONFIG.AIMBOT_KEYBIND = key  
        saveConfig()  
    end)
    aimbotKeybind.LayoutOrder = 2

    local spinbotKeybind = createKeybindButton(scroll, "Spin Bot Keybind", CONFIG.SPINBOT_KEYBIND, function(key)
        CONFIG.SPINBOT_KEYBIND = key
        saveConfig()
    end)
    spinbotKeybind.LayoutOrder = 3


    local lockTargetKeybind = createKeybindButton(scroll, "Lock Target Keybind", CONFIG.LOCK_TARGET_KEYBIND, function(key)
        CONFIG.LOCK_TARGET_KEYBIND = key
        saveConfig()
    end)
    lockTargetKeybind.LayoutOrder = 5
    
    local autoMedusaKeybind = createKeybindButton(scroll, "Auto Medusa Keybind", CONFIG.AUTO_MEDUSA_KEYBIND, function(key)
        CONFIG.AUTO_MEDUSA_KEYBIND = key
        saveConfig()
    end)
    autoMedusaKeybind.LayoutOrder = 6

    local instantGrabKeybind = createKeybindButton(scroll, "Instant Grab Keybind", CONFIG.INSTANT_GRAB_KEYBIND, function(key)
        CONFIG.INSTANT_GRAB_KEYBIND = key
        saveConfig()
    end)
    instantGrabKeybind.LayoutOrder = 7

    local dropBrainrotKeybind = createKeybindButton(scroll, "Drop Brainrot Keybind", CONFIG.DROP_BRAINROT_KEYBIND, function(key)
        CONFIG.DROP_BRAINROT_KEYBIND = key
        saveConfig()
    end)
    dropBrainrotKeybind.LayoutOrder = 8

    local floatKeybind = createKeybindButton(scroll, "Float Keybind", CONFIG.FLOAT_KEYBIND, function(key)
        CONFIG.FLOAT_KEYBIND = key
        saveConfig()
    end)
    floatKeybind.LayoutOrder = 9

    local autoPlayKeybind = createKeybindButton(scroll, "Auto Play Keybind", CONFIG.AUTO_PLAY_KEYBIND, function(key)
        CONFIG.AUTO_PLAY_KEYBIND = key
        saveConfig()
    end)
    autoPlayKeybind.LayoutOrder = 10


    local antiStealKeybind = createKeybindButton(scroll, "Anti Steal Keybind", CONFIG.ANTI_STEAL_KEYBIND, function(key)
        CONFIG.ANTI_STEAL_KEYBIND = key
        saveConfig()
    end)
    antiStealKeybind.LayoutOrder = 12

    local autoWalkKeybind = createKeybindButton(scroll, "Auto Walk Keybind", CONFIG.AUTO_WALK_KEYBIND, function(key)
        CONFIG.AUTO_WALK_KEYBIND = key
        saveConfig()
    end)
    autoWalkKeybind.LayoutOrder = 13

    local flingKeybind = createKeybindButton(scroll, "Fling Keybind", CONFIG.FLING_KEYBIND, function(key)
        CONFIG.FLING_KEYBIND = key
        saveConfig()
    end)
    flingKeybind.LayoutOrder = 13
    ...
    S.UserInputService.InputBegan:Connect(function(input, gpe)
        ...
        if input.UserInputType == Enum.UserInputType.Keyboard then
            if CONFIG.TOGGLE_GUI_KEYBIND and input.KeyCode == CONFIG.TOGGLE_GUI_KEYBIND then
                toggleGui()
            elseif CONFIG.AUTO_BAT_KEYBIND and input.KeyCode == CONFIG.AUTO_BAT_KEYBIND then
            ...
            elseif CONFIG.AIMBOT_KEYBIND and input.KeyCode == CONFIG.AIMBOT_KEYBIND then
                ...
                CONFIG.AIMBOT_ENABLED = newAimState
                aimbotEnabled = newAimState
                saveConfig()
                if CONFIG.AIMBOT_ENABLED then startBodyAimbot() else stopBodyAimbot() end
                if _aimbotBtn then setToggleVisual(_aimbotBtn, newAimState) end
            elseif CONFIG.SPINBOT_KEYBIND and input.KeyCode == CONFIG.SPINBOT_KEYBIND then
                ...
                CONFIG.SPINBOT_ENABLED = newSpinState
                spinbotEnabled = newSpinState
                saveConfig()
                if spinbotEnabled then startSpinBot() else stopSpinBot() end
                if spinbotPanelBtn then
            ...
            elseif CONFIG.LOCK_TARGET_KEYBIND and input.KeyCode == CONFIG.LOCK_TARGET_KEYBIND then
                CONFIG.LOCK_TARGET_ENABLED = not CONFIG.LOCK_TARGET_ENABLED
                lockTargetEnabled = CONFIG.LOCK_TARGET_ENABLED
                saveConfig()
                if lockTargetPanelBtn then
                ...
                if CONFIG.LOCK_TARGET_ENABLED then startLockTarget() else stopLockTarget() end
            elseif CONFIG.AUTO_MEDUSA_KEYBIND and input.KeyCode == CONFIG.AUTO_MEDUSA_KEYBIND then
            ...
            elseif CONFIG.INSTANT_GRAB_KEYBIND and input.KeyCode == CONFIG.INSTANT_GRAB_KEYBIND then
                local newState = not CONFIG.INSTANT_GRAB_ENABLED
                CONFIG.INSTANT_GRAB_ENABLED = newState
                instantGrabEnabled = newState
                saveConfig()
                if newState then 
                    startInstantGrab() 
                else 
            ...
            elseif CONFIG.DROP_BRAINROT_KEYBIND and input.KeyCode == CONFIG.DROP_BRAINROT_KEYBIND then
                executeDrop()
            elseif CONFIG.FLOAT_KEYBIND and input.KeyCode == CONFIG.FLOAT_KEYBIND then
                floatEnabled = not floatEnabled
                if floatPanelBtn then
                    floatPanelBtn.Text = "FLOAT: " .. (floatEnabled and "ON" or "OFF")
                    floatPanelBtn.TextColor3 = COLORS.Accent
                end
                if floatEnabled then startFloat() else stopFloat() end
                saveConfig()
            elseif CONFIG.AUTO_PLAY_KEYBIND and input.KeyCode == CONFIG.AUTO_PLAY_KEYBIND then
                if apGuiInstance and apGuiInstance.Enabled then
                    apLaunchSide(nil)   -- auto-detect side
                end
            elseif CONFIG.ANTI_STEAL_KEYBIND and input.KeyCode == CONFIG.ANTI_STEAL_KEYBIND then
                ...
                if antiStealPanelBtn then
                    antiStealPanelBtn.Text = "ANTI STEAL: " .. (CONFIG.ANTI_STEAL_PANEL_VISIBLE and "ON" or "OFF")
                    antiStealPanelBtn.TextColor3 = COLORS.Accent
                end
                saveConfig()
            elseif CONFIG.FLING_KEYBIND and input.KeyCode == CONFIG.FLING_KEYBIND then
                flingEnabled = not flingEnabled
                ...
                if flingEnabled then startFling() else stopFling() end
                saveConfig()
            elseif CONFIG.AUTO_WALK_KEYBIND and input.KeyCode == CONFIG.AUTO_WALK_KEYBIND then
                awLaunch()
            end
        end
    end)
...
if mainFrame then mainFrame.Visible = (CONFIG.MAIN_GUI_VISIBLE ~= false) end
