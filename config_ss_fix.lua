...
-- Universal config path: works on Krnl, Delta iOS/Android, Synapse, Fluxus, etc.
-- Most executors root writefile/readfile to their own workspace folder.
local CONFIG_FILE = "LooprixDuel.json"

-- Auto-save debounce state
local _savePending   = false
local _SAVE_DEBOUNCE = 0.4  -- seconds

-- ======================================
-- DRAG / CLAMP UTILITY  (shared by every draggable element)
-- ======================================
...
-- Clamps frame so it stays fully inside the viewport.
-- padX/padY add an inner margin (default 4 px each side).
local function clampToScreen(frame, padX, padY)
    padX = padX or 4
    padY = padY or 4
    local vp   = _getViewport()
    local size = frame.AbsoluteSize       -- real pixel dimensions (respects UIScale)
    local pos  = frame.AbsolutePosition   -- real pixel top-left

    local minX = padX
    local minY = padY
    local maxX = vp.X - size.X - padX
    local maxY = vp.Y - size.Y - padY

    local cx = math.clamp(pos.X, minX, maxX)
    local cy = math.clamp(pos.Y, minY, maxY)

    -- Keep scale component, replace offset component
    frame.Position = UDim2.new(
        frame.Position.X.Scale, cx - frame.Position.X.Scale * vp.X,
        frame.Position.Y.Scale, cy - frame.Position.Y.Scale * vp.Y
    )
end

-- Generic drag-and-clamp.  Call once per draggable element.
-- `frame`     : the GuiObject to move
-- `handle`    : the part the user grabs (often == frame)
-- `saveKey`   : string key inside CONFIG._guiPositions to persist position
-- `lockedFn`  : optional function() → bool; if true dragging is blocked
...
local function makeDraggable(frame, handle, saveKey, lockedFn, onSaved)
    ...
    local dragStart = nil
    local startPos  = nil
    local dragInput = nil  -- track the exact InputObject that started the drag
    ...
    handle.InputBegan:Connect(function(inp)
        ...
        -- Sink input so the game camera/player doesn't react
        inp.Handled = true
    end)

    _UIS.InputChanged:Connect(function(inp)
        if not active or lockedFn() then return end
        if inp ~= dragInput then return end
        if not dragStart then return end
        local delta = Vector2.new(inp.Position.X, inp.Position.Y) - dragStart
        if not dragging and delta.Magnitude < 5 then return end
        dragging = true
    ...
    -- Return whether the last gesture was a tap (no drag) so callers can
    -- treat it as a click.
    return function() return not dragging end
end
...
local function saveConfig()
    ...
    _savePending = true
    task.delay(_SAVE_DEBOUNCE, function()
        _savePending = false

        local cfg = {}

        -- Булевые флаги: сохраняем ТОЛЬКО если true; false просто не пишем в файл.
        -- При загрузке отсутствующий ключ = false (скрипт понимает это сам).
        local BOOL_KEYS = {
            "AUTO_BAT_ENABLED","AIMBOT_ENABLED","SPINBOT_ENABLED","ANTIRAGDOLL_ENABLED",
            "UNWALK_ENABLED","ESP_ENABLED","INF_JUMP_ENABLED","OPTIMIZER_ENABLED",
            "XRAY_ENABLED","LOCK_TARGET_ENABLED","AUTO_MEDUSA_ENABLED","INSTANT_GRAB_ENABLED",
            "ANTI_DIE_ENABLED","UI_LOCKED","SPEED_ENABLED","NO_COLLISION_ENABLED",
            "TP_DOWN_ENABLED","AUTO_TP_DOWN_ENABLED","DESYNC_ENABLED",
            "AUTO_REACT_STEAL_ENABLED","NOTIFICATIONS_ENABLED",
            "LOCK_TARGET_PANEL_VISIBLE","DROP_BRAINROT_PANEL_VISIBLE","SPEED_GUI_VISIBLE",
            "FLING_PANEL_VISIBLE","FLOAT_PANEL_VISIBLE","FLOAT_ACTIVE","SPINBOT_PANEL_VISIBLE",
            "TAUNT_PANEL_VISIBLE","ANTI_STEAL_PANEL_VISIBLE","AUTO_LOCK_PANEL_VISIBLE",
            "TP_DOWN_PANEL_VISIBLE","AUTO_WALK_PANEL_VISIBLE","AUTO_PLAY_GUI_VISIBLE",
            "DESYNC_PANEL_VISIBLE","ACTIVE_HUD_VISIBLE","GRAB_BAR_VISIBLE",
        }
        for _, k in ipairs(BOOL_KEYS) do
            if CONFIG[k] == true then cfg[k] = true end
        end
        -- MAIN_GUI_VISIBLE берём из переменной guiVisible
        if guiVisible == true then cfg.MAIN_GUI_VISIBLE = true end

        -- Числовые значения: сохраняем всегда (0 тоже важен)
        cfg.SPEED_VALUE                  = CONFIG.SPEED_VALUE
        cfg.STEAL_SPEED_VALUE            = CONFIG.STEAL_SPEED_VALUE
        cfg.AIMBOT_RANGE                 = CONFIG.AIMBOT_RANGE
        cfg.AIMBOT_DISABLE_RANGE         = CONFIG.AIMBOT_DISABLE_RANGE
        cfg.SPINBOT_SPEED                = CONFIG.SPINBOT_SPEED
        cfg.LOCK_TARGET_SPEED            = CONFIG.LOCK_TARGET_SPEED
        cfg.AUTO_MEDUSA_RANGE            = CONFIG.AUTO_MEDUSA_RANGE
        cfg.INSTANT_GRAB_ACTIVATION_DIST = CONFIG.INSTANT_GRAB_ACTIVATION_DIST
        cfg.FLOAT_HEIGHT                 = CONFIG.FLOAT_HEIGHT
        cfg.FLOAT_SPEED                  = CONFIG.FLOAT_SPEED
        cfg.FOV_VALUE                    = CONFIG.FOV_VALUE
        cfg.GUI_COLOR_R                  = CONFIG.GUI_COLOR_R
        cfg.GUI_COLOR_G                  = CONFIG.GUI_COLOR_G
        cfg.GUI_COLOR_B                  = CONFIG.GUI_COLOR_B
        cfg.UI_SCALE                     = CONFIG.UI_SCALE
        cfg.AUTO_PLAY_DELAY              = CONFIG.AUTO_PLAY_DELAY

        -- Кнопки: сохраняем только если привязка установлена
        local KEYBIND_KEYS = {
            ...
            "DROP_BRAINROT_KEYBIND","FLOAT_KEYBIND","AUTO_PLAY_KEYBIND","FLING_KEYBIND",
            "ANTI_STEAL_KEYBIND","AUTO_LOCK_KEYBIND","AUTO_WALK_KEYBIND",
            "TP_DOWN_KEYBIND","AUTO_REACT_STEAL_KEYBIND",
        }
        for _, k in ipairs(KEYBIND_KEYS) do
            if CONFIG[k] then cfg[k] = CONFIG[k].Name end
        end

        -- GUI позиции / оффсеты: сохраняем только если есть данные
        if CONFIG._guiPositions    then cfg._guiPositions    = CONFIG._guiPositions    end
        if CONFIG._autoPlayOffsets then cfg._autoPlayOffsets = CONFIG._autoPlayOffsets end
        if CONFIG._apBasePositions then cfg._apBasePositions = CONFIG._apBasePositions end

        -- Universal writefile: robust across Krnl / Delta iOS / Delta Android / Synapse / Fluxus
        pcall(function()
            local json = game:GetService("HttpService"):JSONEncode(cfg)
            -- Try standard writefile first
            local ok = pcall(writefile, CONFIG_FILE, json)
            -- Fallback: some executors need the workspace/ prefix
            if not ok then
                pcall(writefile, "workspace/" .. CONFIG_FILE, json)
            end
        end)
    end)
end
...
local function loadConfig()
    -- Universal read: don't rely on isfile() — absent on Delta iOS/Android and some others.
    -- Try the plain filename first, then the workspace/ prefix fallback.
    local rawData
    pcall(function() rawData = readfile(CONFIG_FILE) end)
    if not rawData or rawData == "" then
        pcall(function() rawData = readfile("workspace/" .. CONFIG_FILE) end)
    end
    ...
    applyBool("DESYNC_ENABLED")
    applyBool("AUTO_REACT_STEAL_ENABLED")
    applyBool("NOTIFICATIONS_ENABLED")
    -- panel visibility
    ...
    applyBool("DESYNC_PANEL_VISIBLE")
    ...
    applyNum("FLOAT_HEIGHT"); applyNum("FLOAT_SPEED"); applyNum("FOV_VALUE"); applyNum("GUI_COLOR_R")
    ...
    applyKey("AUTO_PLAY_KEYBIND"); applyKey("FLING_KEYBIND")
    applyKey("ANTI_STEAL_KEYBIND"); applyKey("AUTO_LOCK_KEYBIND")
    applyKey("AUTO_WALK_KEYBIND"); applyKey("TP_DOWN_KEYBIND")
    applyKey("AUTO_REACT_STEAL_KEYBIND")
    -- gui positions
    ...
    if type(cfg._apBasePositions) == "table" then
        CONFIG._apBasePositions = cfg._apBasePositions
        for side, pts in pairs(cfg._apBasePositions) do
            if AP.BASE[side] then
                for key, v in pairs(pts) do
                    if AP.BASE[side][key] ~= nil then
                        AP.BASE[side][key] = Vector3.new(v.x or 0, v.y or 0, v.z or 0)
                    end
                end
            end
        end
...
-- ======================================
-- AUTO PLAY: BACKGROUND BASE SCAN (1 sec)
-- Проверяет _getMyPlot() каждую секунду.
-- Как только база найдена — ставит _plotReady=true и останавливается.
-- Больше не нужно сканировать — уже нашли.
-- ======================================
task.spawn(function()
    while true do
        if AP._plotReady then break end
        task.wait(1)
        pcall(function()
            if _getMyPlot() then
                AP._plotReady = true
...
local function _detectSide()
    ...
    _currentSide  = hrp.Position.Z > 58 and "R" or "L"
    _sideDetected = true
    return _currentSide
end

function GetSide()
    return _currentSide or _detectSide()
end

task.spawn(function()
    _waitMyPlot()
    repeat task.wait()
        if _detectSide() then break end
    until false
end)
...
local function executeDrop()
    ...
    if dropBrainrotPanelBtn then
        dropBrainrotPanelBtn.BackgroundColor3 = COLORS.Accent
    end
...
local function createAutoWalkGui()
    ...
    awMainBtn.MouseButton1Click:Connect(function()
        local side = GetSide()
        if side then awLaunch(side) end
    end)
...
local function canRunAutoPlay()
    ...
    local players = S.Players:GetPlayers()
    if #players ~= 2 then return false, "players" end
    return true, nil
end
...
local function specialRespawn(plr)
...
local function startDesync()
    ...
    local wasAntiDieOn = CONFIG.ANTI_DIE_ENABLED
    if wasAntiDieOn then deactivateAntiDie() end

    pcall(function() raknet.desync(true) end)

    if S.LocalPlayer.Character then
        local hum = S.LocalPlayer.Character:FindFirstChildOfClass("Humanoid")
        if hum then hum.Health = 0 end
        S.LocalPlayer.Character:Destroy()
    end

    task.wait(0.1)
    task.spawn(function()
        specialRespawn(S.LocalPlayer)
        if wasAntiDieOn then
            task.wait(0.4)
            activateAntiDie()
        end
    end)
...
local function apLaunchSide(side)
    ...
    task.spawn(function()
        ...
        for i = 1, 3 do
            local p = _getMyPlot()
            if p then foundPlot = p end
            if i < 3 then task.wait(0.02) end
        end
        if not foundPlot then return end
        ...
        AP.enabled = true
        apStartLoop()
        apSetBtnActive(side == "L" and apBtnL or apBtnR, true)
    end)
end
...
local function createSpeedGui()
    ...
    -- Live-update status button colors when accent changes
    local _statusAccentConn = nil
    local function refreshStatusBtnColor()
        if CONFIG.SPEED_ENABLED then
            statusBtn.BackgroundColor3 = COLORS.Accent
            statusBtn.TextColor3 = Color3.fromRGB(10,10,10)
        else
            statusBtn.BackgroundColor3 = COLORS.Background
            statusBtn.TextColor3 = COLORS.Accent
        end
    end
    -- Hook into applyAccentColor via a tracked frame trick: use a hidden tracker frame
    local _trackerLbl = Instance.new("TextLabel")
    _trackerLbl.AutoLocalize = false
    _trackerLbl.Visible = false
    _trackerLbl.Parent = r0
    table.insert(_accentLabels, { -- watch via RenderStepped instead
    })
    -- Simpler: just poll accent every frame for status button text color
    S.RunService.RenderStepped:Connect(function()
    ...
    makeDraggable(main, main, "speedGui", function() return CONFIG.UI_LOCKED end)
    startSpeed()
...
local function createAutoPlayGui()
    ...
    apMainBtn.MouseButton1Click:Connect(function()
        local side = GetSide()
        if side then apLaunchSide(side) end
    end)
    ...
    -- Drag main
    makeDraggable(main, main, "autoPlayMain", function() return CONFIG.UI_LOCKED end)
...
-- ======================================
-- DESYNC FLOATING GUI  (AutoWalk/AutoPlay style)
-- ======================================
...
local function createDesyncGui()
    ...
    local sg = Instance.new("ScreenGui", S.PlayerGui)
    sg.AutoLocalize = false
    sg.Name = "Looprix_DesyncGui"
    sg.ResetOnSpawn = false
    sg.DisplayOrder = 999998
    sg.Enabled = CONFIG.DESYNC_PANEL_VISIBLE
    ...
    local savedDS = CONFIG._guiPositions and CONFIG._guiPositions.desyncPanel
    if savedDS then
        main.Position = UDim2.new(savedDS.scaleX, savedDS.offsetX, savedDS.scaleY, savedDS.offsetY)
    else
        main.Position = UDim2.new(0, 10, 0, 280)
    end
    ...
    titleLbl.AutoLocalize = false
    titleLbl.Size = UDim2.new(1,-34,1,0); titleLbl.Position = UDim2.new(0,20,0,0)
    titleLbl.BackgroundTransparency = 1; titleLbl.Text = "Desync"
    titleLbl.TextColor3 = COLORS.Accent; titleLbl.Font = Enum.Font.GothamSemibold
    trackLabel(titleLbl)
    titleLbl.TextSize = 11; titleLbl.TextXAlignment = Enum.TextXAlignment.Left
    local tg = Instance.new("UIGradient", titleLbl)
    ...
    -- Two-button row: [DESYNC toggle] [RESPAWN]
    local btnRow = Instance.new("Frame", content)
    btnRow.Size = UDim2.new(1, 0, 1, 0)
    btnRow.BackgroundTransparency = 1
    btnRow.BorderSizePixel = 0

    local HALF = UDim2.new(0.5, -3, 1, 0)

    desyncPanelBtn = Instance.new("TextButton", btnRow)
    desyncPanelBtn.AutoLocalize = false
    desyncPanelBtn.Size = HALF
    desyncPanelBtn.Position = UDim2.new(0, 0, 0, 0)
    desyncPanelBtn.BackgroundColor3 = CONFIG.DESYNC_ENABLED and COLORS.Accent or COLORS.Surface
    desyncPanelBtn.BackgroundTransparency = CONFIG.DESYNC_ENABLED and 0.0 or COLORS.SurfaceTransparency
    desyncPanelBtn.Text = CONFIG.DESYNC_ENABLED and "DESYNC ON" or "DESYNC"
    desyncPanelBtn.TextColor3 = CONFIG.DESYNC_ENABLED and Color3.fromRGB(10,10,10) or COLORS.TextDim
    desyncPanelBtn.Font = Enum.Font.GothamBold; desyncPanelBtn.TextSize = 11; desyncPanelBtn.BorderSizePixel = 0
    Instance.new("UICorner", desyncPanelBtn).CornerRadius = UDim.new(0,7)
    makeAS(desyncPanelBtn, 1, 0.45)

    local respawnBtn = Instance.new("TextButton", btnRow)
    respawnBtn.AutoLocalize = false
    respawnBtn.Size = HALF
    respawnBtn.Position = UDim2.new(0.5, 3, 0, 0)
    respawnBtn.BackgroundColor3 = COLORS.Surface
    respawnBtn.BackgroundTransparency = COLORS.SurfaceTransparency
    respawnBtn.Text = "RESPAWN"
    respawnBtn.TextColor3 = COLORS.TextDim
    respawnBtn.Font = Enum.Font.GothamBold; respawnBtn.TextSize = 11; respawnBtn.BorderSizePixel = 0
    Instance.new("UICorner", respawnBtn).CornerRadius = UDim.new(0,7)
    makeAS(respawnBtn, 1, 0.45)

    if CONFIG.DESYNC_ENABLED then
        startDesync()
    end

    desyncPanelBtn.MouseButton1Click:Connect(function()
        local newState = not CONFIG.DESYNC_ENABLED
        toggleDesync(newState)
        S.TweenService:Create(desyncPanelBtn, tweenInfoFast, {
            BackgroundColor3       = newState and COLORS.Accent or COLORS.Surface,
            BackgroundTransparency = newState and 0.0 or COLORS.SurfaceTransparency,
        }):Play()
        desyncPanelBtn.TextColor3 = newState and Color3.fromRGB(10,10,10) or COLORS.TextDim
        desyncPanelBtn.Text = newState and "DESYNC ON" or "DESYNC"
        showNotification("Desync", newState)
    end)

    respawnBtn.MouseButton1Click:Connect(function()
        S.TweenService:Create(respawnBtn, tweenInfoFast, {
            BackgroundColor3 = COLORS.Accent, BackgroundTransparency = 0.0,
        }):Play()
        respawnBtn.TextColor3 = Color3.fromRGB(10,10,10)
        task.delay(0.4, function()
            S.TweenService:Create(respawnBtn, tweenInfoFast, {
                BackgroundColor3 = COLORS.Surface, BackgroundTransparency = COLORS.SurfaceTransparency,
            }):Play()
            respawnBtn.TextColor3 = COLORS.TextDim
        end)
        task.spawn(function()
    ...
    -- Drag
    local dsDrag,dsDS,dsDSP=false,nil,nil
    main.InputBegan:Connect(function(inp)
        if CONFIG.UI_LOCKED then return end
        if inp.UserInputType==Enum.UserInputType.MouseButton1 or inp.UserInputType==Enum.UserInputType.Touch then
            dsDrag=false; dsDS=inp.Position; dsDSP=main.Position
        end
    end)
    S.UserInputService.InputChanged:Connect(function(inp)
        if CONFIG.UI_LOCKED or not dsDS then return end
        if inp.UserInputType==Enum.UserInputType.MouseMovement or inp.UserInputType==Enum.UserInputType.Touch then
            if (inp.Position-dsDS).Magnitude>5 then
                dsDrag=true
                local d=inp.Position-dsDS
                main.Position=UDim2.new(dsDSP.X.Scale,dsDSP.X.Offset+d.X,dsDSP.Y.Scale,dsDSP.Y.Offset+d.Y)
            end
        end
    end)
    main.InputEnded:Connect(function(inp)
        if inp.UserInputType==Enum.UserInputType.MouseButton1 or inp.UserInputType==Enum.UserInputType.Touch then
            if dsDrag then
                CONFIG._guiPositions = CONFIG._guiPositions or {}
                CONFIG._guiPositions.desyncPanel = {
                    scaleX=main.Position.X.Scale, scaleY=main.Position.Y.Scale,
                    offsetX=main.Position.X.Offset, offsetY=main.Position.Y.Offset,
                }
                saveConfig()
            end
            dsDrag=false; dsDS=nil
        end
    end)
...
local function createSettingsTab(parent)
    ...
    local cfgActionsContent, cfgActionsWrapper = createCategory(scroll, "[Config]", true)
    cfgActionsWrapper.LayoutOrder = 5
    ...
    local resetBtn = createElement("TextButton", {
        ...
        BorderSizePixel = 0,
        LayoutOrder = 1,
        Parent = cfgActionsContent,
    })
    createElement("UICorner", { CornerRadius = UDim.new(0, 6), Parent = resetBtn })
    createElement("UIStroke", { Color = Color3.fromRGB(255, 80, 80), Thickness = 1, Transparency = 0.3, ApplyStrokeMode = Enum.ApplyStrokeMode.Border, Parent = resetBtn })
    trackLabel(resetBtn)
    resetBtn.MouseButton1Click:Connect(function()
        stopAllSystems()
        -- Non-default toggles → false
        CONFIG.AUTO_BAT_ENABLED = false
        CONFIG.AIMBOT_ENABLED = false
        CONFIG.SPINBOT_ENABLED = false
        CONFIG.ANTIRAGDOLL_ENABLED = false
        CONFIG.UNWALK_ENABLED = false
        CONFIG.ESP_ENABLED = false
        CONFIG.INF_JUMP_ENABLED = false
        CONFIG.OPTIMIZER_ENABLED = false
        CONFIG.XRAY_ENABLED = false
        CONFIG.LOCK_TARGET_ENABLED = false
        CONFIG.AUTO_MEDUSA_ENABLED = false
        CONFIG.INSTANT_GRAB_ENABLED = false
        CONFIG.UI_LOCKED = false
        CONFIG.SPEED_ENABLED = false
        CONFIG.NO_COLLISION_ENABLED = false
        CONFIG.TP_DOWN_ENABLED = false
        CONFIG.AUTO_TP_DOWN_ENABLED = false
        CONFIG.DESYNC_ENABLED = false
        CONFIG.NOTIFICATIONS_ENABLED = false
        CONFIG.AUTO_REACT_STEAL_ENABLED = false
        autoReactStealEnabled = false
        stopAutoReactStealWatcher()
        -- Default toggles (true by default) — keep ON
        CONFIG.ANTI_DIE_ENABLED = true
        antiDieEnabled = true
        CONFIG.GRAB_BAR_VISIBLE = true
        CONFIG.MAIN_GUI_VISIBLE = true
        -- Numeric defaults
        CONFIG.SPEED_VALUE = 60
        CONFIG.STEAL_SPEED_VALUE = 30
        CONFIG.AIMBOT_RANGE = 40
        CONFIG.AIMBOT_DISABLE_RANGE = 45
        CONFIG.SPINBOT_SPEED = 30
        CONFIG.LOCK_TARGET_SPEED = 55
        CONFIG.AUTO_MEDUSA_RANGE = 5
        CONFIG.INSTANT_GRAB_ACTIVATION_DIST = 8
        CONFIG.FLOAT_HEIGHT = 10
        CONFIG.FLOAT_SPEED = 50
        CONFIG.FOV_VALUE = 70
        CONFIG.AUTO_PLAY_DELAY = 0.03
        -- Panel visibility → all false
        CONFIG.LOCK_TARGET_PANEL_VISIBLE = false
        CONFIG.DROP_BRAINROT_PANEL_VISIBLE = false
        CONFIG.SPEED_GUI_VISIBLE = false
        CONFIG.FLING_PANEL_VISIBLE = false
        CONFIG.FLOAT_PANEL_VISIBLE = false
        CONFIG.FLOAT_ACTIVE = false
        CONFIG.SPINBOT_PANEL_VISIBLE = false
        CONFIG.TAUNT_PANEL_VISIBLE = false
        CONFIG.ANTI_STEAL_PANEL_VISIBLE = false
        CONFIG.AUTO_LOCK_PANEL_VISIBLE = false
        CONFIG.TP_DOWN_PANEL_VISIBLE = false
        CONFIG.AUTO_WALK_PANEL_VISIBLE = false
        CONFIG.AUTO_PLAY_GUI_VISIBLE = false
        CONFIG.DESYNC_PANEL_VISIBLE = false
        CONFIG.ACTIVE_HUD_VISIBLE = false
        saveConfig()
        S.TweenService:Create(resetBtn, tweenInfoFast, { BackgroundColor3 = Color3.fromRGB(255, 80, 80) }):Play()
        task.delay(0.25, function()
            S.TweenService:Create(resetBtn, tweenInfoFast, { BackgroundColor3 = COLORS.Surface }):Play()
        end)
    end)
...
local function createBindsTab(parent)
    ...
    kb("Auto Play",         "AUTO_PLAY_KEYBIND",        10)
    kb("Auto Walk",         "AUTO_WALK_KEYBIND",        11)
    kb("Fling",             "FLING_KEYBIND",           12)
    kb("Anti Steal",        "ANTI_STEAL_KEYBIND",      13)
    kb("Auto Lock",         "AUTO_LOCK_KEYBIND",       14)
    kb("Tp Down",           "TP_DOWN_KEYBIND",         15)
    kb("Auto React Steal",  "AUTO_REACT_STEAL_KEYBIND", 16)
...
local function createDuelTab(parent)
    ...
    local floatPanelToggle = createToggle(mobContent, "Float", CONFIG.FLOAT_PANEL_VISIBLE, function(state)
        ...
        if state then
            -- Панель включена — запускаем систему если FLOAT_ACTIVE
            if CONFIG.FLOAT_ACTIVE then startFloat() end
        else
            -- Панель выключена — система останавливается (лидирует панель)
    ...
    local autoPlayGuiToggle = createToggle(toolsContent, "Auto Play", CONFIG.AUTO_PLAY_GUI_VISIBLE, function(state)
        ...
        if not state and AP.enabled then
            apStopLoop()
        end
    end)
    ...
    local desyncToggle = createToggle(toolsContent, "Desync", CONFIG.DESYNC_PANEL_VISIBLE, function(state)
    ...
    local autoReactToggle = createToggle(toolsContent, "Auto React Steal", CONFIG.AUTO_REACT_STEAL_ENABLED, function(state)
    ...
    autoReactToggle.LayoutOrder = 11
...
local function createGui()
    ...
    S.UserInputService.InputBegan:Connect(function(input, gpe)
        ...
        elseif CONFIG.FLOAT_KEYBIND and kc == CONFIG.FLOAT_KEYBIND then
            ...
            saveConfig()
        elseif CONFIG.AUTO_PLAY_KEYBIND and kc == CONFIG.AUTO_PLAY_KEYBIND then
            if apGuiInstance and apGuiInstance.Enabled then
                local side = GetSide()
                if side then apLaunchSide(side) end
            end
        elseif CONFIG.AUTO_WALK_KEYBIND and kc == CONFIG.AUTO_WALK_KEYBIND then
        ...
        elseif CONFIG.TP_DOWN_KEYBIND and kc == CONFIG.TP_DOWN_KEYBIND then
            tpDown()
        elseif CONFIG.AUTO_REACT_STEAL_KEYBIND and kc == CONFIG.AUTO_REACT_STEAL_KEYBIND then
...
local function createStatsIsland()
    ...
    S.RunService.RenderStepped:Connect(function()
        ...
        pbBorderGrad.Rotation    = (t * 80) % 360
    end)
