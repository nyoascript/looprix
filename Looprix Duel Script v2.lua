...
local function migrateConfig(savedConfig)
    if not savedConfig._version or savedConfig._version < 3 then
        ...
        if not savedConfig.gui.walkPanel then
            savedConfig.gui.walkPanel = {
                scaleX = 0,
                scaleY = 0,
                offsetX = savedConfig.WALK_PANEL_X or 0,
                offsetY = savedConfig.WALK_PANEL_Y or 0,
                visible = savedConfig.WALK_PANEL_VISIBLE or false
            }
        end
        ...
        if not savedConfig.gui.spinbotPanel then
...
local function buildSerializableConfig()
    ...
    ser.gui.walkPanel = CONFIG._guiPositions and CONFIG._guiPositions.walkPanel or { scaleX = 0, scaleY = 0, offsetX = 0, offsetY = 0, visible = false }
    ...
    ser.toggles = {
        ...
        AUTO_WALK_ENABLED = CONFIG.AUTO_WALK_ENABLED,
        ...
        WALK_PANEL_VISIBLE = CONFIG.WALK_PANEL_VISIBLE,
    ...
    local keybindFields = {
        ...
        "AUTO_WALK_KEYBIND", "LOCK_TARGET_KEYBIND", "AUTO_MEDUSA_KEYBIND", "INSTANT_GRAB_KEYBIND", 
        "DROP_BRAINROT_KEYBIND", "FLOAT_KEYBIND", "AUTO_PLAY_L_KEYBIND", "AUTO_PLAY_R_KEYBIND",
        "ANTI_STEAL_KEYBIND"
    }
...
local function loadSerializedConfig(saved)
    if saved.gui then
        ...
        if saved.gui.walkPanel then
            CONFIG._guiPositions = CONFIG._guiPositions or {}
            CONFIG._guiPositions.walkPanel = saved.gui.walkPanel
            CONFIG.WALK_PANEL_VISIBLE = saved.gui.walkPanel.visible or false
        end
...
local walkPanelGui, walkPanelBtn
...
local function startAutoWalk()
    ...
    autoWalkConnection = S.RunService.Heartbeat:Connect(function()
        if not CONFIG.AUTO_WALK_ENABLED then return end
        local c = character
        if not c then return end
        ...
        if autoWalkPhase == 1 then
            ...
            h.AssemblyLinearVelocity = Vector3.new(moveDir.X * 30, h.AssemblyLinearVelocity.Y, moveDir.Z * 30)
        ...
        elseif autoWalkPhase == 2 then
            ...
            if dist < 1 then
                ...
                CONFIG.AUTO_WALK_ENABLED = false
                saveConfig()
                if walkPanelBtn then
                    walkPanelBtn.Text = "WALK: OFF"
                    walkPanelBtn.TextColor3 = COLORS.Accent
                end
    ...
    addConnection(autoWalkConnection)
end
...
local function apSetBtnActive(btn, active)
    ...
    S.TweenService:Create(btn, tweenInfoFast, {
        BackgroundColor3       = active and Color3.fromRGB(10, 40, 28) or COLORS.Surface,
        BackgroundTransparency = active and 0 or COLORS.SurfaceTransparency,
    }):Play()
    btn.TextColor3 = active and COLORS.Accent or COLORS.TextDim
end
...
local function apLaunchSide(side, btn, otherBtn)
    if AP.enabled and AP.activeSide == side then
        -- FIX: single stop — apStopLoop restores speed
        apStopLoop()
        apSetBtnActive(btn, false)
    else
        -- FIX: stop first (restores speed), then start fresh
        apStopLoop()
        AP.activeSide = side
        apSetBtnActive(btn, true)
        apSetBtnActive(otherBtn, false)
        apRefreshESP()
        AP.enabled = true
        apStartLoop()
    end
...
local function createAutoPlayGui()
    ...
    local BW = (W - PAD*2 - GAP) / 2
    local function makeSideBtn(label, lo)
        local btn = Instance.new("TextButton", content)
        btn.LayoutOrder = lo; btn.Size = UDim2.new(0, BW, 0, BTN_H)
        btn.BackgroundColor3 = COLORS.Surface
        btn.BackgroundTransparency = COLORS.SurfaceTransparency
        btn.Text = label; btn.TextColor3 = COLORS.TextDim
        btn.Font = Enum.Font.GothamBold; btn.TextSize = 10; btn.BorderSizePixel = 0
        Instance.new("UICorner", btn).CornerRadius = UDim.new(0,7)
        makeAccentStroke(btn, 1, 0.45)
        return btn
    end
    apBtnL = makeSideBtn("< Auto L", 1)
    apBtnR = makeSideBtn("Auto R >", 2)

    local function makeBadge(parent, cfgKey)
        local badge = Instance.new("TextLabel", parent)
        badge.Size = UDim2.new(0,32,0,13); badge.AnchorPoint = Vector2.new(1,0)
        badge.Position = UDim2.new(1,-3,1,-6)
        badge.BackgroundColor3 = COLORS.Background; badge.BackgroundTransparency = 0.15
        badge.BorderSizePixel = 0; badge.Font = Enum.Font.GothamBold; badge.TextSize = 8
    ...
    local badgeL = makeBadge(apBtnL, "AUTO_PLAY_L_KEYBIND")
    local badgeR = makeBadge(apBtnR, "AUTO_PLAY_R_KEYBIND")
    task.spawn(function()
        while main and main.Parent do
            task.wait(0.5)
            badgeL.Text = CONFIG.AUTO_PLAY_L_KEYBIND and CONFIG.AUTO_PLAY_L_KEYBIND.Name or "NONE"
            badgeR.Text = CONFIG.AUTO_PLAY_R_KEYBIND and CONFIG.AUTO_PLAY_R_KEYBIND.Name or "NONE"
        end
    end)

    apBtnL.MouseButton1Click:Connect(function() apLaunchSide("L", apBtnL, apBtnR) end)
    apBtnR.MouseButton1Click:Connect(function() apLaunchSide("R", apBtnR, apBtnL) end)
...
local function setupFloatingPanels()
    walkPanelGui, walkPanelBtn = createFloatingPanel("Walk", "walkPanel", "WALK_PANEL_VISIBLE", "WALK: " .. (CONFIG.AUTO_WALK_ENABLED and "ON" or "OFF"))

    if CONFIG.AUTO_WALK_ENABLED then walkPanelBtn.TextColor3 = COLORS.Accent end

    walkPanelBtn.Activated:Connect(function()
        CONFIG.AUTO_WALK_ENABLED = not CONFIG.AUTO_WALK_ENABLED
        autoWalkEnabled = CONFIG.AUTO_WALK_ENABLED
        walkPanelBtn.Text = "WALK: " .. (autoWalkEnabled and "ON" or "OFF")
        walkPanelBtn.TextColor3 = COLORS.Accent
        saveConfig()
...
local function createBindsTab(parent)
    ...
    local autoPlayLKeybind = createKeybindButton(scroll, "Auto Play L Keybind", CONFIG.AUTO_PLAY_L_KEYBIND, function(key)
        CONFIG.AUTO_PLAY_L_KEYBIND = key
        saveConfig()
    end)
    autoPlayLKeybind.LayoutOrder = 10

    local autoPlayRKeybind = createKeybindButton(scroll, "Auto Play R Keybind", CONFIG.AUTO_PLAY_R_KEYBIND, function(key)
        CONFIG.AUTO_PLAY_R_KEYBIND = key
        saveConfig()
    end)
    autoPlayRKeybind.LayoutOrder = 11
...
local function createDuelTab(parent)
    ...
    local walkPanelToggle = createToggle(mobContent, "Auto Walk", CONFIG.WALK_PANEL_VISIBLE, function(state)
        CONFIG.WALK_PANEL_VISIBLE = state; saveConfig()
        if walkPanelGui then walkPanelGui.Enabled = state end
    end)
    walkPanelToggle.LayoutOrder = 4
    ...
    S.UserInputService.InputBegan:Connect(function(input, gpe)
        ...
        if input.UserInputType == Enum.UserInputType.Keyboard then
            ...
            elseif CONFIG.AUTO_WALK_KEYBIND and input.KeyCode == CONFIG.AUTO_WALK_KEYBIND then
                ...
                saveConfig()
                if walkPanelBtn then
                    walkPanelBtn.Text = "WALK: " .. (CONFIG.AUTO_WALK_ENABLED and "ON" or "OFF")
                    walkPanelBtn.TextColor3 = COLORS.Accent
                end
                if CONFIG.AUTO_WALK_ENABLED then startAutoWalk() else stopAutoWalk() end
            ...
            elseif CONFIG.FLOAT_KEYBIND and input.KeyCode == CONFIG.FLOAT_KEYBIND then
                ...
                if floatEnabled then startFloat() else stopFloat() end
                saveConfig()
            elseif CONFIG.AUTO_PLAY_L_KEYBIND and input.KeyCode == CONFIG.AUTO_PLAY_L_KEYBIND then
                if apGuiInstance and apGuiInstance.Enabled then
                    apLaunchSide("L", apBtnL, apBtnR)
                end
            elseif CONFIG.AUTO_PLAY_R_KEYBIND and input.KeyCode == CONFIG.AUTO_PLAY_R_KEYBIND then
                if apGuiInstance and apGuiInstance.Enabled then
                    apLaunchSide("R", apBtnR, apBtnL)
                end
...
local function createActiveHud()
    ...
    local TOGGLE_DEFS = {
        ...
        { k = "ESP_ENABLED",               n = "ESP"            },
        { k = "AUTO_WALK_ENABLED",         n = "Auto Walk"      },
